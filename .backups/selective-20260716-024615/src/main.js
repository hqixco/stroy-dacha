...
function sanitizePhoneDigits(value) {
  ...
  for (const digit of digits) {
    if (!localDigits.length && (digit === '7' || digit === '8')) {
      continue;
    }
...
function getPhoneValidationError(value) {
  ...
  const localDigits = digits.length > 10 && (digits.startsWith('7') || digits.startsWith('8'))
    ? digits.slice(1)
    : digits;

  if (localDigits[0] === '7' || localDigits[0] === '8') {
    return 'Проверьте номер телефона: после +7 нельзя вводить 7 или 8.';
  }

  if (!validatePhone(value)) {
    return 'Проверьте номер телефона: заполните его полностью в формате +7 (___) ___-__-__.';
  }
...
function handleGenericFormSubmit(form, eventName, extraPayload = {}) {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const phoneValidationError = phone ? getPhoneValidationError(phone.value) : '';

    if (phoneValidationError) {
      showFormMessage(form, phoneValidationError, 'error');
      phone.focus();
      return;
    }
...
function bindCalculator() {
  ...
  roots.calculatorNext.addEventListener('click', async () => {
    ...
    if (phoneValidationError) {
      showFormMessage(roots.calculatorForm, phoneValidationError, 'error');
      phone.focus();
      return;
    }
